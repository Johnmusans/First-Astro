import { c as createComponent, r as renderHead, e as renderTemplate, b as addAttribute } from '../chunks/astro/server_CIE0xyp6.mjs';
import 'piccolore';
import 'clsx';
/* empty css                                  */
...
</p> ${allTags.length > 0 && renderTemplate`<div class="flex flex-wrap gap-2 mt-8"> ${allTags.map((tag) => renderTemplate`<span class="text-[12px] tracking-[0.05em] uppercase text-off-black border border-off-black/60 rounded-full px-3 py-1">${tag}</span>`)} </div>`} </section> <!-- Posts grid --> <section class="max-w-[1432px] mx-auto px-6 md:px-10 pb-24"> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5"> ${posts.map((post, i) => renderTemplate`<a${addAttribute(`/blog/${post.slug}`, "href")}${addAttribute(`group block rounded-[32px] p-5 transition-colors ${i === 0 ? "md:col-span-2 bg-atmosphere-wash" : "border border-pale-stone/20 hover:border-off-black"}`, "class")}> ${post.data.cover && renderTemplate`<div${addAttribute(`rounded-[20px] overflow-hidden bg-paper-canvas/60 mb-5 ${i === 0 ? "aspect-[21/9]" : "aspect-[16/9]"}`, "class")}> <img${addAttribute(post.data.cover, "src")}${addAttribute(post.data.title, "alt")} loading="lazy" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"> </div>`} <div class="flex flex-wrap gap-2 mb-3"> ${post.data.tags.map((tag) => renderTemplate`<span class="text-[12px] tracking-[0.05em] uppercase text-off-black border border-off-black/60 rounded-full px-3 py-1">${tag}</span>`)} </div> <h2${addAttribute(`font-display text-ink mb-2 leading-[1.2] ${i === 0 ? "text-[32px] md:text-[40px]" : "text-[22px]"}`, "class")}> ${post.data.title} </h2> <p class="text-[14px] text-pale-stone leading-[1.5] mb-4">${post.data.description}</p> <span class="text-[12px] tracking-[0.05em] uppercase text-faint-text"> ${post.data.date.toLocaleDateString("fr-FR", { year: "numeric", month: "long", day: "numeric" })} </span> </a>`)} </div> </section> <!-- Footer --> <footer class="border-t border-pale-stone/20"> <div class="max-w-[1432px] mx-auto px-6 md:px-10 py-10 flex flex-col sm:flex-row justify-between gap-4 text-[12px] tracking-[0.05em] uppercase text-faint-text"> <p>© 2025 John Musans</p> <a href="/" class="hover:text-ink">← Accueil</a> </div> </footer> </body></html>`;
